...
class KUKACommunicationProtocol : Base
{
    ...
    internal override List<string> GetActionMessages(Action action, RobotCursor cursor)
    {
        ...
        switch (action.Type)
        {
            ...
            case ActionType.Transformation:
                MakeXmlForTRTActions(action, cursor);
                break;

            // @TODO: complete rest of actions

            case ActionType.Axes:
                MakeXmlForAxes(action, cursor);
                break;

            case ActionType.Message:
                MakeXmlForMessage(action);
                break;

            case ActionType.Wait:
                MakeXmlForWait(action);
                break;

            case ActionType.Speed:
                MakeXmlForSpeed(action, cursor);
                break;

            case ActionType.Acceleration:
                MakeXmlForAcceleration(action, cursor);
                break;

            case ActionType.Precision:
                MakeXmlForPrecision(action, cursor);
                break;


            case ActionType.DefineTool:
                MakeXmlForDefinTool(action);
                break;

            case ActionType.MotionMode:
                MakeXmlForMotionMode(action);
                break;

            case ActionType.AttachTool:

                MakeXmlForAttachTool(action, cursor);
                break;

            case ActionType.DetachTool:
                MakeXmlForDetachTool(action);
                break;

            case ActionType.IODigital:
                MakeXmlForIODigital(action);
                break;
    ...
    private void MakeXmlForIODigital(Action action)
    {
    ...
    private void MakeXmlForDetachTool(Action action)
    {
        xml += string.Format(CultureInfo.InvariantCulture,
           "<A ID=\"{0}\" T=\"{1}\" V1=\"{2}\" V2=\"{3}\" V3=\"{4}\" V4=\"{5}\" V5=\"{6}\" V6=\"{7}\" V7=\"{8}\"/>",
           action.Id,
    ...
    private void MakeXmlForAttachTool(Action action, RobotCursor cursor)
    {
        // @To Do
        ...
        YawPitchRoll euler = cursor.tool.TCPOrientation.ToQuaternion().ToYawPitchRoll();
        // PTP/LIN X Y Z A B C 
        xml += string.Format(CultureInfo.InvariantCulture,
            "<A ID=\"{0}\" T=\"{1}\" V1=\"{2}\" V2=\"{3}\" V3=\"{4}\" V4=\"{5}\" V5=\"{6}\" V6=\"{7}\" V7=\"{8}\"/>",
            action.Id,
    ...
    private void MakeXmlForMotionMode(Action action)
    {
    ...
    private void MakeXmlForDefinTool(Action action)
    {
        // Add a log here to avoid a confusing default warning.
        Logger.Verbose("`DefineTool()` doesn't need to be streamed.");
        xml += string.Format(CultureInfo.InvariantCulture,
    ...
    private void MakeXmlForPrecision(Action action, RobotCursor cursor)
    {
        xml += string.Format(CultureInfo.InvariantCulture,
            "<A ID=\"{0}\" T=\"{1}\" V1=\"{2}\" V2=\"{3}\" V3=\"{4}\" V4=\"{5}\" V5=\"{6}\" V6=\"{7}\" V7=\"{8}\"/>",
            action.Id,
    ...
    private void MakeXmlForAcceleration(Action action, RobotCursor cursor)
    {
        xml += string.Format(CultureInfo.InvariantCulture,
            "<A ID=\"{0}\" T=\"{1}\" V1=\"{2}\" V2=\"{3}\" V3=\"{4}\" V4=\"{5}\" V5=\"{6}\" V6=\"{7}\" V7=\"{8}\"/>",
            action.Id,
    ...
    private void MakeXmlForSpeed(Action action, RobotCursor cursor)
    {
    ...
    private void MakeXmlForWait(Action action)
    {
        // WaitTime T
    ...
    private void MakeXmlForMessage(Action action)
    {
    ...
    private void MakeXmlForAxes(Action action, RobotCursor cursor)
    {
    ...
    private void MakeXmlForTRTActions(Action action, RobotCursor cursor)
    {
        // adjusting the action's rotation to match the KUKA robot's convention
        cursor.rotation.RotateLocal(new Rotation(0, 1, 0, 90));
    ...
    #region Commented Code From Switch Statement
